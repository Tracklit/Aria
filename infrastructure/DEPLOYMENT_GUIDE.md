...
cd aria-app
docker build -f Dockerfile.server -t $ACR_NAME.azurecr.io/aria-mobile-backend:latest .
docker push $ACR_NAME.azurecr.io/aria-mobile-backend:latest
```
...
| `aria-app/Dockerfile.server` | Node.js API Dockerfile |
