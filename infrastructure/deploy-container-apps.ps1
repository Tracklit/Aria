...
if (-not $SkipContainerBuild) {
    ...
    Write-Host "  cd aria-app && docker build -f Dockerfile.server -t <acr-name>.azurecr.io/aria-mobile-backend:latest ."
    Write-Host "  docker push <acr-name>.azurecr.io/aria-mobile-backend:latest"
}
