...
3. **Set Up Database**
   ...
   cd aria-api && python scripts/run_migrations.py
...
4. **Start Redis**
   ...
   cd aria-api && docker-compose up -d redis
   ```
...
6. **Start Services**
   ...
   **Option B: Manual (3 terminals)**
   ```bash
   # Terminal 1: FastAPI
   cd aria-api && python src/main.py
...
cd aria-api && pytest tests/test_ai_logic.py -v
